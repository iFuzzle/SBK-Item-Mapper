...
local CURRENT_ITEM_ADDR = 0x122295 -- 0x122292 for Red Items
local FRAMES_NEEDED = 20
...
local function test_combination(pos, next_item)
    ...
    console.log(string.format(
        "Pos: 0x%02X | Next: %03d → Item: 0x%02X",
        pos, next_item, item
    ))
...
for _, pos in ipairs(positions) do
    ...
    for item_dec = 0, 255 do
        local success, item = pcall(test_combination, pos, item_dec)
        if success then
...
-- CSV-Export
local file = io.open("item_map_blue_final.csv", "w")
file:write("Position_Hex;NextItem_Dec;Item_Hex\n")
...
console.log("Mapping erfolgreich! Datei: item_map_blue_final.csv")
